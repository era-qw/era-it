...
type CanvasWithCleanup = HTMLCanvasElement & { cleanupFuzzyText?: () => void }

/** Числовой размер из CSS-строки вида clamp(2rem, 8vw, 8rem) — canvas .font не понимает clamp(). */
...
/**
 * Текст с эффектом «расфокусированных строк»: каждая строка canvas-текста
 * хаотично сдвигается по горизонтали. Интенсивность растёт при наведении.
 */
export default function FuzzyText({
  ...
  useEffect(() => {
    ...
    let glitchEndTimeoutId: number | undefined
    let clickTimeoutId: number | undefined
    const canvas = canvasRef.current
    if (!canvas) return

    /* пауза, когда canvas вне вьюпорта — не тратим кадры впустую */
    let visible = true
    ...
    const init = async () => {
      ...
      ;(canvas as CanvasWithCleanup).cleanupFuzzyText = cleanup
    }
    ...
    return () => {
      ...
      if (canvas) {
        ;(canvas as CanvasWithCleanup).cleanupFuzzyText?.()
      }
    }
  }, [
    children,
