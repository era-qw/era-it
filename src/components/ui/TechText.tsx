...
/**
 * Технический canvas-текст: в покое по нему проходит «линза», проявляющая
 * пунктирные контуры букв; при наведении буква получает рамку-селектор
 * с размерами, буквы можно перетаскивать (пружина возвращает на место).
 */
export default function TechText({
  ...
  const settingsRef = useRef<TechSettings | null>(null)
  ...
  useEffect(() => {
    ...
    const family = (s: TechSettings) =>
      s.fontFamily || getComputedStyle(container).fontFamily || 'sans-serif'
    const fontFor = (s: TechSettings, size: number) => `${s.fontWeight} ${size}px ${family(s)}`

    const setFont = (target: CanvasRenderingContext2D, s: TechSettings, size: number) => {
      target.font = fontFor(s, size)
      const withSpacing = target as CanvasRenderingContext2D & { letterSpacing?: string }
      if ('letterSpacing' in withSpacing) withSpacing.letterSpacing = `${s.letterSpacing * size}px`
      target.textAlign = 'left'
      target.textBaseline = 'alphabetic'
    }

    const sprite = (s: TechSettings, view: WordLayout, glyph: Glyph['box'] & { char?: string }, glyphFull: { char: string; x: number }, stroke: boolean): Sprite => {
      const pad = Math.ceil(s.strokeWidth * 2 + 4)
      const left = glyph.x1 - pad
      const top = glyph.y1 - pad
      const w = glyph.x2 - glyph.x1 + pad * 2
      const h = glyph.y2 - glyph.y1 + pad * 2
      ...
      if (stroke) {
        ...
        c.strokeText(glyphFull.char, glyphFull.x, view.baseline)
        c.setLineDash([])
        c.globalCompositeOperation = 'destination-out'
        c.fillStyle = '#000000'
        c.fillText(glyphFull.char, glyphFull.x, view.baseline)
    ...
    const ensureLayout = (s: TechSettings): WordLayout => {
      const key = [
        s.text,
        family(s),
        s.fontWeight,
      ...
      chars.forEach((char, i) => {
        ...
        glyphs.push({
          ...
          outline: 0,
          index: i,
          fill: sprite(s, next, base.box, base, false),
          dashes: sprite(s, next, base.box, base, true),
        })
      })
      dragging = -1
      frame.index = -1
    ...
    const drawReveal = (s: TechSettings) => {
    ...
    const perimeterPoint = (distance: number, w: number, h: number): number[] => {
    ...
    const drawSpecks = (s: TechSettings, a: number) => {
    ...
    const drawFrame = (s: TechSettings) => {
      ...
      const label =
        moved > 1
          ...
          : `${glyph.char}  ${Math.round(glyph.box.x2 - glyph.box.x1)} × ${Math.round(
              glyph.box.y2 - glyph.box.y1,
            )}`
    ...
    const tick = (now: number) => {
      ...
      if (s.draggable)
        container.style.cursor =
          dragging >= 0 ? 'grabbing' : focus >= 0 && pointer.inside ? 'grab' : ''
